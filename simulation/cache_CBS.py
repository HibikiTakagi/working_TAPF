...
class CBSPolicy:
    def __init__(self, env:MultiRobotsEnvironment):
        ...
        self.cbs = CBS(self.env.connect_dict, self.env.num_agents, self.env.node_distances)
    ...
    def greedy(self, curr_state, dest_state):
        ...
        if refing_flag:
            cbs_path = self.cbs.find_paths(curr_state, dest_state)
        else:
...
class CBS:
    def __init__(self, graph, num_agents, distances) -> None:
        self.graph = graph
        ...
        while open_list and cnt < 10*self.num_agents:
            ...
            for constraint in constraints:
                ...
                if new_node:
                    ...
                    priority += 1
                    heapq.heappush(open_list, (new_cost, priority, new_node))
        
        if (open_list is not None) and len(open_list)!=0:
            _, _, node = heapq.heappop(open_list)
            return node['paths']
        else:
            return gabage
